...
// Stoolap WASM vs SQLite WASM — Pure Go Benchmark
//
// Two engines compared, both via wazero (zero CGO):
//   - Stoolap WASM: stoolap-go/wasm driver
//   - SQLite WASM:  ncruces/go-sqlite3 driver
package main
...
func fmtUs(us float64) string { return fmt.Sprintf("%15.3f", us) }
...
func main() {
	...
	fmt.Println("Stoolap WASM vs SQLite WASM — Pure Go Benchmark (database/sql)")
	fmt.Printf("Configuration: %d rows, %d iterations per test\n", rowCount, iterations)
	fmt.Println("Both engines use database/sql with CGO_ENABLED=0 — fair comparison")
	...
	// --- SQLite WASM setup (ncruces/go-sqlite3) ---
	...
	sInsert, _ := sTx.PrepareContext(ctx, "INSERT INTO users VALUES ($1, $2, $3, $4, $5, $6, $7)")
	for _, u := range users {
	...
	// INSERT single
	sSt, _ = sdb.PrepareContext(ctx, "INSERT INTO users VALUES ($1, $2, $3, $4, $5, $6, $7)")
	...
	sTx, _ = sdb.BeginTx(ctx, nil)
	sOrdIns, _ := sTx.PrepareContext(ctx, "INSERT INTO orders VALUES ($1, $2, $3, $4, $5)")
	...
	fmt.Println()
	fmt.Println("NOTES:")
	fmt.Println("- Both engines use database/sql with CGO_ENABLED=0")
	fmt.Println("- Stoolap: stoolap-go/wasm driver via wazero runtime")
